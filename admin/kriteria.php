  ...
  <body class="hold-transition sidebar-mini layout-fixed">
    ...
    <script>
      ...
      $(document).on("click", "#update", function () {
        var data = new Object();
        data.id = $("#id").val();
        data.kode = $("#kodeKriteria").val();
        data.nama = $("#namaKriteria").val();
        data.bobot = $("#bobotKriteria").val();
        data.jenis = $("#jenisKriteria").val();
        console.log(data);
        $.ajax({
          url: "../backend/updateKriteria.php",
          type: "POST",
          ...
          success: function (resp) {
            ...
            if (result != false) {
              Toast.fire({
                icon: 'success',
                title: 'Berhasil mengubah kriteria.'
              })	
            } else {
              alert("Gagal ubah");
            }
      ...
      $(document).on("click", "#hapusKriteria", function () {
        var data = new Object();
        data.id = $(this).attr("idKriteria");
        console.log(data);
        $.ajax({
          url: "../backend/deleteKriteria.php",
          type: "POST",
          data: JSON.stringify(data),
          cache: false,
          success: function (resp) {
      ...
      $(document).on("click", "#editKriteria", function () {
        ...
        $.ajax({
          url: "../backend/getKriteriaById.php",
          type: "POST",
          data: JSON.stringify(data),
          cache: false,
          success: function (resp) {
            ...
            resultTag =
              ...
              '<i class="fa-solid fa-pen-to-square"></i> Edit Data Kriteria' +
              "</h3>" +
              "</div>" +
              ...
              '<input type="text" class="form-control" id="kodeKriteria" placeholder="Kode Kriteria" value="' +
              result[0].kode +
              '" />' +
              "</div>" +
              '<div class="form-group">' +
              '<label for="namaKriteria">Nama Kriteria</label>' +
              '<input type="text" class="form-control" id="namaKriteria" placeholder="Nama Kriteria" value="' +
              result[0].nama +
              '" />' +
              "</div>" +
              "</div>" +
              ...
              '<input type="number" class="form-control" id="bobotKriteria" placeholder="Bobot" value="' +
              result[0].bobot +
              '" />' +
              "</div>" +
              '<div class="form-group">' +
              '<label for="kodeKriteria">Jenis</label>' +
              '<select class="form-control" id="jenisKriteria">' +
              '<option value="Benefit"' +
              (result[0].jenis == "Benefit" ? "selected" : "") +
              ">Benefit</option>" +
              '<option value="Cost"' +
              (result[0].jenis == "Cost" ? "selected" : "") +
              ">Cost</option>" +
              "</select>" +
              "</div>" +
              "</div>" +
      ...
      $(document).on("click", "#simpan", function () {
        var kriteria = new Object();
        kriteria.kode = $("#kodeKriteria").val();
        kriteria.nama = $("#namaKriteria").val();
        kriteria.bobot = $("#bobotKriteria").val();
        kriteria.jenis = $("#jenisKriteria").val();

        console.log(kriteria);
        $.ajax({
          url: "../backend/addKriteria.php",
          type: "POST",
          data: JSON.stringify(kriteria),
          cache: false,
          success: function (resp) {
            console.log(resp);
            Toast.fire({
                icon: 'success',
                title: 'Berhasil menambah kriteria.'
              })
          },
          error: function () {
            console.log("error");
      ...
      var formTambah =
        ...
        '<div class="form-group">' +
        '<label for="kodeKriteria">Kode Kriteria</label>' +
        '<input type="text" class="form-control" id="kodeKriteria" placeholder="Kode Kriteria" />' +
        "</div>" +
        '<div class="form-group">' +
        '<label for="namaKriteria">Nama Kriteria</label>' +
        '<input type="text" class="form-control" id="namaKriteria" placeholder="Nama Kriteria" />' +
        "</div>" +
        "</div>" +
        '<div class="col-md-6">' +
        '<div class="form-group">' +
        '<label for="namaKriteria">Bobot</label>' +
        '<input type="number" class="form-control" id="bobotKriteria" placeholder="Bobot" />' +
        "</div>" +
        '<div class="form-group">' +
        '<label for="kodeKriteria">Jenis</label>' +
        '<select class="form-control" id="jenisKriteria">' +
      ...
      function reset() {
        ...
        document.getElementById("jenisKriteria").value = "Benefit";
      }
      ...
      $(document).on("click", "#kembali", function () {
        taghtml =
          ...
          '<div class="col-md-6">' +
          '<h3 class="card-title mt-2">' +
          '<i class="fa-solid fa-table"></i> Data Kriteria' +
          "</h3>" +
          "</div>" +
          '<div class="col-md-6 d-flex justify-content-end">' +
            ...
            if (result != false) {
              for (i = 0; i < result.length; i++) {
                ...
                resultTag +=
                  ...
                  "</td>" +
                  "<td>" +
                  '<button class="btn-xs btn-primary" id="editKriteria" idKriteria="' +
                  result[i].id +
                  '">' +
                  '<i class="fa-solid fa-pen-to-square"></i>' +
                  "</button>" +
                  ' <button class="btn-xs btn-danger" id="hapusKriteria" idKriteria="' +
                  result[i].id +
                  '">' +
